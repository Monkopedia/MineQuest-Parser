...
public class RequirementListHandler extends FieldHandler {
	
	private List<RequirementHandler> handlers;

	public RequirementListHandler(QuestLine line, LineDisplay display,
			FieldDefinition fDef) {
		super(line, display, fDef);
		String[] ids = line.fields[index].split(",");
		handlers = new ArrayList<RequirementHandler>();
		for (String id : ids) {
			handlers.add(new RequirementHandler(line, display, fDef, id));
		}
	}

	@Override
	public void createDisplay() {
		for (RequirementHandler handler : handlers) {
		...
		display.button("+", new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				display.index = index;
				handlers.add(new RequirementHandler(line, display, fDef, "1"));
				display.reDraw();
			}
		}, 650, 45, 25);
		display.button("-", new ActionListener() {
