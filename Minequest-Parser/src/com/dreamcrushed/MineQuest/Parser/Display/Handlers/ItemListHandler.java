...
public class ItemListHandler extends FieldHandler {
	
	private List<ItemHandler> handlers;

	public ItemListHandler(QuestLine line, LineDisplay display,
			FieldDefinition fDef) {
		super(line, display, fDef);
		String[] ids = line.fields[index].split(",");
		handlers = new ArrayList<ItemHandler>();
		for (String id : ids) {
			handlers.add(new ItemHandler(line, display, fDef, id));
		}
	}
	...
	public void createDisplay() {
		...
		display.button("+", new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				display.index = index;
				handlers.add(new ItemHandler(line, display, fDef, "1"));
				display.reDraw();
			}
		}, 650, 45, 25);
		display.button("-", new ActionListener() {
