...
public class VectorLocHandler extends FieldHandler {

	private JTextField[] textFields;
	private FloatLocHandler fHandler;

	public VectorLocHandler(QuestLine line, LineDisplay display, FieldDefinition fDef) {
		super(line, display, fDef);
		fHandler = findFHandler(line, display);
	}

	private FloatLocHandler findFHandler(QuestLine line, LineDisplay display) {
		for (FieldHandler handler : display.handlers) {
			if (handler instanceof FloatLocHandler) {
				return (FloatLocHandler)handler;
			}
		}
		return null;
	}
		...
		display.button("GET", new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				final Message message = new Message("Getting next loc from Server", "Info", 200,
								display.frame, display.frame.getX() + display.frame.getWidth() / 2,
								display.frame.getY() + display.frame.getHeight() / 2);
				FloatLoc.setLocListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent e) {
						...
						for (int i = 0; i < 3; i++) {
							float f = 0;
							try {
								f = Float.parseFloat(fHandler.textFields[i].getText());
							} catch (Exception e1) {
							}
							
							textFields[i].setText((FloatLoc.location[i] - f)+ "");
						}
					}
				});
				...
				try {
					networkManager.sendPacket(new FloatLocReq());
