...
public class TargetListHandler extends FieldHandler {
	
	private List<TargetHandler> handlers;

	public TargetListHandler(QuestLine line, LineDisplay display,
			FieldDefinition fDef) {
		super(line, display, fDef);
		String[] ids = line.fields[index].split(",");
		handlers = new ArrayList<TargetHandler>();
		for (String id : ids) {
			handlers.add(new TargetHandler(line, display, fDef, id));
		}
	}
	...
	public void createDisplay() {
		...
		display.button("+", new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				display.index = index;
				handlers.add(new TargetHandler(line, display, fDef, "1"));
				display.reDraw();
			}
		}, 650, 45, 25);
		display.button("-", new ActionListener() {
